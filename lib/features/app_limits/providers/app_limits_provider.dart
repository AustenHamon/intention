...
Future<void> updateLimit(String packageName, int newLimitMinutes) async {
  final index = _appLimits.indexWhere((a) => a.packageName == packageName);
  if (index == -1) return;
  final updated = _appLimits[index].copyWith(dailyLimitMinutes: newLimitMinutes);
  _appLimits[index] = updated;
  await _repository.updateAppLimit(updated);
  await TriggerService.instance.refreshMonitoredPackages();
  notifyListeners();
}
...
Future<void> toggleApp(String packageName) async {
  final index = _appLimits.indexWhere((a) => a.packageName == packageName);
  if (index == -1) return;
  final updated = _appLimits[index].copyWith(isEnabled: !_appLimits[index].isEnabled);
  _appLimits[index] = updated;
  await _repository.updateAppLimit(updated);
  await TriggerService.instance.refreshMonitoredPackages();
  notifyListeners();
